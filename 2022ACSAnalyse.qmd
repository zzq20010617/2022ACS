...
The R programming language (@talia), dplyr(@dplyr), and (@tibble) were used to estimate data and generate tables, package (@knitr) and (@kablee) is used for adding caption to table. The data were gathered from @IPUMS.
```{r}
#| include: false
#| warning: false
#| message: false
...
kable(combined_data, caption = "estimate and actual respondents of each state in USA 2022")
```
